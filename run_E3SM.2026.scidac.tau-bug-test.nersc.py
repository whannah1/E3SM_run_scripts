...
acct = 'm4310'
src_dir = os.getenv('HOME')+'/E3SM/E3SM_SRC2' # branch => whannah/eamxx/create-gwd-atm-proc-rebase w/ mods
...
stop_opt,stop_n,resub,walltime = 'nsteps',5,0,'0:30:00' ; queue = 'debug'
...
prfx = '2026-tau-bug-test-00'
# grid = 'ne4pg2_oQU480'
# grid = 'ne30pg2_r05_IcoswISC30E3r5'

add_case(prefix=prfx,grid='ne4pg2_oQU480',compset='F20TR',num_nodes=1,use_fix=False)
add_case(prefix=prfx,grid='ne4pg2_oQU480',compset='F20TR',num_nodes=1,use_fix=True)
...
def write_atm_namelist(opts):
   ...
   file.write(" fincl2 = 'PS','PSL','Z3','T','U','V','OMEGA','CLDLIQ','CLDICE','PRECT'")
   file.write(         ",'BUTGWSPEC','UTGWORO'")
   # file.write(         ",'UTGWSPEC','FRONTGF'")
...
def main(opts):
   ...
   if config :
      ...
      #-------------------------------------------------------------------------
      if opts.get('use_fix'):
         run_cmd('./xmlchange --id CAM_CONFIG_OPTS --append --val \" -cppdefs \' -DUSE_TAU_FIX \'  \" ')
      #-------------------------------------------------------------------------
