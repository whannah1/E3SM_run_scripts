...
# stop_opt,stop_n,resub,walltime = 'ndays',30,0,'3:00:00'
stop_opt,stop_n,resub,walltime = 'ndays',100,9-1,'4:00:00'
...
def main(compset,msst,arch):
   ...
   case_list = ['E3SM','2024-RCEMIP2',compset_str]

   if debug_mode: case_list.append('debug')
   case = '.'.join(case_list)

   use_cosp = False if 'MMF' in compset else True
   ...
   if config :
      ...
      file=open('user_nl_eam','w');file.write(get_atm_nl_opts(compset,use_cosp));file.close()
      #-------------------------------------------------------
      if use_cosp:
   ...
   print(f'\n  case : {case}\n')
#---------------------------------------------------------------------------------------------------
def get_hist_opts(compset,use_cosp):
   hist_opts = ''
...
#---------------------------------------------------------------------------------------------------
def get_atm_nl_opts(compset,use_cosp):
   hist_opts = get_hist_opts(compset,use_cosp)
   nl_opts = ''
   if use_cosp:
