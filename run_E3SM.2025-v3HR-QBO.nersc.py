...
src_dir = os.getenv('HOME')+'/E3SM/E3SM_SRC1' # master @ April 9 2025 + 4dd681a66aecc3b48ecdb0e21314fff7bb85cca2
...
#-------------------------------------------------------------------------------
def get_run_opts(num_nodes):
   queue,stop_opt,stop_n,resub,walltime = None,None,None,None,None
   if num_nodes==75:
      # queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',1,0,'0:30:00' # HR - 75 nodes
      ...
      # queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',365,4-1,'12:00:00' # 75 nodes
   # if num_nodes==256:
   ...
   if num_nodes==16:
      queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',5,0,'0:30:00' # LR - 32 nodes
   if num_nodes==32:
      queue,stop_opt,stop_n,resub,walltime = 'regular','ndays',5,0,'0:30:00' # LR - 32 nodes
   ...
   return queue,stop_opt,stop_n,resub,walltime
#-------------------------------------------------------------------------------

compset ='F20TR'
# grid = 'ne120pg2_r025_RRSwISC6to18E3r5'
# num_nodes = 75
...
# add_case(prefix='2025-v3HR-QBO-00', beres='new', gweff=0.1, cfrac=2.5, hdpth=1.00) # ??? - never ran these
# add_case(prefix='2025-v3HR-QBO-00', beres='new', gweff=0.1, cfrac=5.0, hdpth=1.00) # ??? - never ran these
# add_case(prefix='2025-v3HR-QBO-00', beres='new', gweff=0.1, cfrac=2.5, hdpth=1.00) # ??? - never ran these
...
# add_case(prefix='2025-v3HR-QBO-01',grid='ne120pg2_r025_RRSwISC6to18E3r5', orogw='old') # control
# add_case(prefix='2025-v3HR-QBO-01',grid='ne120pg2_r025_RRSwISC6to18E3r5', orogw='new') # 
# add_case(prefix='2025-v3HR-QBO-01',grid='ne120pg2_r025_RRSwISC6to18E3r5', orogw='new') # 

### refocus on disabling frontal GWD
# add_case(prefix='2025-v3HR-QBO-00', grid=grid, frontgw='off', beres='old', gweff=0.1, cfrac=2.5, hdpth=0.50) # control
# add_case(prefix='2025-v3HR-QBO-00', grid=grid, frontgw='off', beres='new', gweff=0.1, cfrac=2.5, hdpth=1.00, effgw_cm=0.5)
# add_case(prefix='2025-v3HR-QBO-00', grid=grid, frontgw='off') 

### get quick output of frontogenesis function
...
#---------------------------------------------------------------------------------------------------
def main(opts):
   global compset#, num_nodes

   grid_short = None
   if 'ne30pg2'  in opts['grid']: grid_short = 'ne30pg2'
   if 'ne120pg2' in opts['grid']: grid_short = 'ne120pg2'

   case_list = ['E3SM']
   for key,val in opts.items(): 
      if key in ['prefix']:
         case_list.append(val)
      elif key in ['grid'] and grid_short is not None:
   ...
   atm_ntasks = max_mpi_per_node*int(opts['num_nodes'])
   ...
   if submit :
      ...
      nfile = 'user_nl_eam'
      file = open(nfile,'w') 
      if 'ne30pg2'  in opts['grid']: file.write(v3LR_atm_opts)
      if 'ne120pg2' in opts['grid']: file.write(v3HR_atm_opts)
      ...
      if 'effgw_cm'         in opts.keys(): file.write(f' effgw_cm                     = {opts["effgw_cm"]} \n')
      if 'frontgfc'         in opts.keys(): file.write(f' frontgfc                     = {opts["frontgfc"]} \n')
      if 'tom_sponge_start' in opts.keys(): file.write(f' tom_sponge_start             = {opts["tom_sponge_start"]} \n')
      ...
      if 'ne30pg2'  in opts['grid']: file.write(v3LR_lnd_opts)
      if 'ne120pg2' in opts['grid']: file.write(v3HR_lnd_opts)
      file.close()
      #-------------------------------------------------------------------------
      ...
      #-------------------------------------------------------------------------
      queue,stop_opt,stop_n,resub,walltime = get_run_opts(opts['num_nodes'])
      #-------------------------------------------------------------------------
...
v3LR_atm_opts = f'''
 ...
 fincl1 = 'AODALL','AODBC','AODDUST','AODPOM','AODSO4','AODSOA','AODSS','AODVIS',
          ...
          'PUTEND','BUTGWSPEC','UTGWORO','UTGWSPEC'
          'Uzm','Vzm','Wzm','THzm','VTHzm','WTHzm','UVzm','UWzm','THphys','PSzm'

 !fincl2 = 'PS','FLUT','PRECT','U850','U600','FRONTGF','FRONTGFA'

 phys_grid_ctem_zm_nbas = 120
 phys_grid_ctem_za_nlat = 90
 phys_grid_ctem_nfreq = -1

 bnd_topo='/global/cfs/cdirs/e3sm/inputdata/atm/cam/topo/USGS-gtopo30_ne30np4pg2_x6t-SGH_forOroDrag.c20241001.nc'
...
v3HR_atm_opts = '''
 ...
 use_od_ss=.true.
 use_od_fd=.true.
 bnd_topo='/global/cfs/cdirs/e3sm/inputdata/atm/cam/topo/USGS-gtopo30_ne120np4pg2_x6t_forOroDrag.c20241019.nc'
