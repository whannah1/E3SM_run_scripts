#!/usr/bin/env python3
#---------------------------------------------------------------------------------------------------
import os, datetime, subprocess as sp, hashlib, json
...
#---------------------------------------------------------------------------------------------------
clean_exe,create_exe,config_exe,build_exe,newcase,config,set_opts,submit,continue_run = False,False,False,False,False,False,False,False,False
...
acct = 'E3SM_Dec'
src_dir = os.getenv('HOME')+'/E3SM/E3SM_SRC0' # branch => master @ 2025-10-07
...
with open('/lus/flare/projects/E3SM_Dec/whannah/LH_sampling_base10.json', 'r') as file:
   LHS_parameter_values = json.load(file)
   num_case,num_param  = len(LHS_parameter_values),len(LHS_parameter_values[0])
#---------------------------------------------------------------------------------------------------
add_case(exe=True, prefix='2025-EACB',grid='ne256',num_nodes=64)
# add_case(exe=True, prefix='2025-EACB',grid='ne256',num_nodes=32)
# add_case(exe=True, prefix='2025-EACB',grid='ne128',num_nodes=16)
# add_case(exe=True, prefix='2025-EACB',grid='ne64', num_nodes=8)
# add_case(exe=True, prefix='2025-EACB',grid='ne32', num_nodes=4)
#---------------------------------------------------------------------------------------------------
# num_case = 1 # use first case for testing
...
lnd_init_root  = None # '/global/cfs/cdirs/m4310/whannah/E3SM/init_data/v3.LR.amip_0101/archive/rest/2000-01-01-00000'
lnd_init_file  = None # f'{init_root}/v3.LR.amip_0101.elm.r.2000-01-01-00000.nc'
lnd_data_root  = f'{din_loc_root}/lnd/clm2/surfdata_map'
lnd_data_file  = f'{lnd_data_root}/surfdata_0.25x0.25_simyr2010_c240206_TOP.nc'
lnd_luse_file  = f'{lnd_data_root}/landuse.timeseries_r025_CMIP7_HIST_simyr1850-2024_c250723.nc'
#---------------------------------------------------------------------------------------------------
...
def get_case_name(opts,exe=False):
   ...
   for key,val in opts.items(): 
      if key in ['prefix','grid']:  case_list.append(val)
...
def run_ens_member(opts):   
   ...
   #----------------------------------------------------------------------------
   print(f'\n  case     : {case}')
   print(  f'  root     : {case_root.replace(case,"")}')
   print(  f'  exe_root : {exe_root}'); print()
   #------------------------------------------------------------------------------------------------
   if newcase :
      if os.path.isdir(case_root): exit(f'\n{clr.RED}This case already exists!{clr.END}\n')
      cmd = f'{src_dir}/cime/scripts/create_newcase'
   ...
   if set_opts :
      #-------------------------------------------------------------------------
      # handle namelist options here
      write_lnd_nl_opts()
      # run_cmd(f'./atmchange p3::set_cld_frac_r_to_one=true ') # this should be the default
      #-------------------------------------------------------------------------
      ...
      add_hist_file('1da_ne30pg2.yaml',hist_opts_1da_ne30pg2)
