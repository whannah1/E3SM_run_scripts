...
src_dir  = f'{top_dir}/E3SM_SRC0/' # branch => master @ Aug 4 2026
...
init_file = '/pscratch/sd/w/whannah/files_init/HICCUP.atm_era5.2022-08-26.00.ne30np4.L80.nc'
...
def main(opts):
   ...
   case_root = os.getenv('SCRATCH')+f'/scream_scratch/pm-cpu/{case}'
   #------------------------------------------------------------------------------------------------
   if newcase :
      ...
      # cmd += f' --machine=dane'
      cmd += f' -mach pm-cpu -compiler gnu'
      cmd += f' --pecount {atm_ntasks}x{atm_nthrds} --project {acct}'
      run_cmd(cmd)
   #------------------------------------------------------------------------------------------------
   ...
   if config :
      ...
      #-------------------------------------------------------------------------
      if opts.get('prefix')=='2026-HC-TEST-01':
   ...
   if submit :
      ...
      # file.write(f' tom_sponge_start=0.0 \n') # default
      file.write(f' tom_sponge_start=15 \n')
      file.close()
      #-------------------------------------------------------------------------
      if opts.get('compset'):
         # run_cmd(f'./xmlchange RUN_STARTDATE=2022-08-26')
         run_cmd(f'./xmlchange RUN_STARTDATE=2012-08-26')
      #-------------------------------------------------------------------------
