...
#SBATCH --qos=regular
#SBATCH --job-name=generate_topo_test_120
...
# Specify root paths
  grid_root=${SCRATCH}/test_files_grid
  map_root=${SCRATCH}/test_files_map
  topo_root=${SCRATCH}/test_files_topo
...
topo_file_a=${topo_root}/tmp_USGS-topo-cube${NE_SRC}.nc
topo_file_b=${topo_root}/tmp_USGS-topo-cube${NE_SRC}_remap_ne${NE_DST}pg2.nc
...
map_file_src_to_pg2=${map_root}/map_ne${NE_SRC}pg1_to_ne${NE_DST}pg2_fv2fv_flx.nc
#---------------------------------------------------------------------------  
...
time ncremap ${MAP_ARGS} -a fv2se_flx \
  ...
  --tmp_dir=${map_root}
#---------------------------------------------------------------------------
# Create map from source to target pg2
time ncremap ${MAP_ARGS} -a fv2fv_flx \
  --src_grd=${grid_root}/scrip_ne${NE_SRC}pg1.nc \
  --dst_grd=${grid_root}/scrip_ne${NE_DST}pg2.nc \
  --map_file=${map_file_src_to_pg2} \
  --tmp_dir=${map_root}
#---------------------------------------------------------------------------
...
#---------------------------------------------------------------------------
# Compute SGH with NCO

# calculate square of terrain height on source grid - skip this step in the future
cp ${topo_file_0} ${topo_file_a}
ncap2 -A -s 'terr2=terr^2' ${topo_file_a} ${topo_file_a}
ncatted -a units,terr2,m,c,“m^2” ${topo_file_a}

# remap to target pg2 grid
ncremap -v terr,terr2,SGH30,LANDFRAC -m ${map_file_src_to_pg2} -i ${topo_file_a} -o ${topo_file_b}
ncrename -d grid_size,ncol ${topo_file_b}

# append remapped terr data to 
ncks -A -v terr,terr2,SGH30 ${topo_file_b} ${topo_file_2}

# Calculate SGH
ncap2 -A -s 'SGH=sqrt( (terr2*9.80616^2) - 2*PHIS*(terr*9.80616) + PHIS^2 ) / 9.80616' ${topo_file_2}
ncatted -a units,SGH,m,c,'m' ${topo_file_2}
ncatted -a long_name,SGH,a,c,'std deviation of 3km cubed-sphere elevation' ${topo_file_2}

# Create final file
ncks -C -O -x -v terr,terr2,lat_vertices,lon_vertices ${topo_file_2} ${topo_file_3}
