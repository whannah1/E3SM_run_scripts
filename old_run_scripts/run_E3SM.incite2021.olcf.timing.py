...
#-------------------------------------------------------------------------------
# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'FC5AV1C-L','GNUCPU', 64, 0, 0,False,False

# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU', 64,32, 1,True,True
compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU',127,32, 1,True,True

# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU', 64,64, 1,True,True
# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU',127,64, 1,True,True
# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU',254,64, 1,True,True

# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU',1013,64,64,True,True
# compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX',  'GNUGPU',1013,32,32,True,True

# ne,npg=4,2; compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX','GNUGPU',8,32,32,True,True
# ne,npg=4,2; compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX','GNUGPU',8,64,64,True,True
# ne,npg=4,2; compset,arch,num_nodes,crm_nx,crm_ny,use_momfb,use_vt = 'F-MMFXX','GNUGPU',4,64,64,True,True
#-------------------------------------------------------------------------------
...
if ne==4: grid = f'{res}_{res}' 


if arch=='GNUCPU' : task_per_node = 84
if arch=='GNUGPU' : task_per_node = 24#12

# if arch=='GNUGPU' : task_per_node =  6
# if arch=='GNUGPU' : task_per_node = 24
# if arch=='GNUGPU' : task_per_node = 42

   
# case_list = ['E3SM','ESMT_EXP_TIMING',arch,grid,compset]

### after rebasing to get changes from PR #4333
# case_list = ['E3SM','INCITE2021-THRD_TIMING_TEST',arch,grid,compset,f'TPN_{task_per_node}','NTHRDS_1'] 
case_list = ['E3SM','INCITE2021-THRD_TIMING_TEST',arch,grid,compset,f'TPN_{task_per_node}','NTHRDS_2'] 

if 'MMF' in compset:
   case_list.append(f'NXY_{crm_nx}x{crm_ny}')
   if use_momfb: case_list.append('MOMFB')
   if use_vt   : case_list.append('BVT')
if case_list[1]=='ESMT_EXP_TIMING':
   case_list.append(f'NODES_{num_nodes}')
   case_list.append(f'TPN_{task_per_node}')
   case_list.append(f'RADNX_{rad_nx}')
   if 'MMF' in compset:
...
for p in params:
   if p[0]=='L': 
      if p[1]!='72': init_file_atm = f'HICCUP.cami_mam3_Linoz_ne{ne}np4.L{p[1]}_c20210623.nc'
...
if config :
   ...
   if 'MMF' in compset:
      ...
      run_cmd(f'./xmlchange --append -id CAM_CONFIG_OPTS -val \" -crm_dx 2000 \" ')
      if use_vt: 
...
if submit : 
   ...
   if not print_commands_only: 
      ...
      # file.write(          ",'UBP','VBP'")
      # file.write(          ",'UAP','VAP'")
      # file.write(          ",'TAUTMSX','TAUTMSY'")        # 2D turbulent mountain stress tendencies
      ...
      if 'dtime' in locals():
         if dtime == 20*60 :
            ...
            #...except for hypervis_subcycle_q
            ...
            if ne==4 :
               ...
               file.write(f'hypervis_subcycle_q = 2 \n')
