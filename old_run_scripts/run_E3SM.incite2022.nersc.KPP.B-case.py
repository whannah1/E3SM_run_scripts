...
src_dir  = os.getenv('HOME')+'/E3SM/E3SM_SRC4/' # branch => whannah/mmf/KPP-dev
...
# stop_opt,stop_n,resub,walltime = 'nstep',8,0,'0:03:00'
stop_opt,stop_n,resub,walltime = 'nstep',8,0,'0:30:00'
...
compset,arch,num_nodes,disable_output = 'WCYCL1950-MMF1','GNUCPU',512, True # try MMF w/ CPU
...
if newcase :
   ...
   if arch=='GNUCPU' : cmd += f' -mach pm-cpu -compiler gnu    -pecount {total_ntasks}x{total_nthrds} '
   if arch=='GNUGPU' : cmd += f' -mach pm-gpu -compiler gnugpu -pecount {total_ntasks}x{total_nthrds} '
   run_cmd(cmd)
