...
# hypothesis: memory is a problem for large crm domain, so switch to single precision
...
for (res, dx, nx) in itertools.product(gcm_res, crm_dx, crm_dims):
    ...
    if Config.config is True:
        ...
        BuildCase.set_ntasks_in_env_mach_pes(test=trial_run, num_phys=21600, num_dyn=21600)
    ...
    if Config.build is True:
        ...
        BuildCase.set_cam_config_opts(test=trial_run, default_set="SP1",
                                      **{"-crm_nx": str(nx[0]), "-crm_ny": str(nx[-1]),
                                         ...
                                         "-crm_dt": str(5),
                                         "-microphys": "mg2",
                                         "-cppdefs": "'-DCRM_SINGLE_PRECISION -DSP_DIR_NS'"})
