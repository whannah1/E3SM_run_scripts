...
Config = runsp.Config(newcase=False, config=False, clean=True, build=True,
                      runsim=False, update_namelist=True, debug=True,
                      copyinit=True, testrun=trial_run, hindcast=True,
                      sp=True)

BuildCase = runsp.Case(case_name="ne16_SP1_32x4_gnu_dt10", res="ne16", sp=Config.sp,
                       compiler="gnu")
...
ref_dir = ("/lustre/atlas/proj-shared/csc249/crjones/acme_hindcast_ics/"
           "nudge_erai_uv_above_pbl_ne16/")
ref_case = "ACME_ZM_nlev72_ne16_nudge_above_pbl"
...
for case, ndays in ndays_to_run.iteritems():
    ...
    if Config.runsim is True:
        RunCase.submit(trial_run, '--mail-user', 'christopher.jones@pnnl.gov', '-M', 'fail', '-M', 'end')
