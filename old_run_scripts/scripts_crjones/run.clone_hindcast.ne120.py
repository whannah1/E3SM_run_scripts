...
BuildCase = runsp.Case(case_name="ne120_SP1_16x1_intel",
...
if Config.config is True:
    ...
    BuildCase.set_ntasks_in_env_mach_pes(test=trial_run, num_phys=21600, 
                                         num_dyn=21600, num_threads=1)
...
if Config.build is True:
    ...
    BuildCase.set_cam_config_opts(test=trial_run, default_set="SP1",
                                  **{"-crm_nx": "16", "-crm_dx": "1000",
                                     "-crm_dt": "10", "-microphys": "mg2"})
...
if not BuildCase.is_build_complete():
    sys.exit("Build did not complete successfully. Exiting now.")
...
ref_dir = ("/lustre/atlas/proj-shared/csc249/crjones/acme_hindcast_ics/"
           "nudge_erai_uv_above_pbl_ne120/")
ref_case = "ACME_ZM_CTL_ne120_nudge_above_pbl"
...
ndays_to_run = {'20110430': 5,
                ...
                '20110827': 5}
if quick_test:
    ndays_to_run = {'20110329': 2}  # simple test run
for case, ndays in ndays_to_run.iteritems():
    ...
    if Config.update_namelist is True:
        ...
        RunCase.update_namelist(ncdata=ncdata(case_date, ref_dir, ref_case), 
                                ...
                                se_nsplit="6",
                                rsplit="2",
                                qsplit="1")  # (maybe add cld_macmic_num_steps=3, too)
