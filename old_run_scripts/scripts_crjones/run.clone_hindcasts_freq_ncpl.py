...
# run_combos = [(32, 1, 4, False)]
run_combos = [(256, 1, 1, False)]
atm_ncpl = 192

def gen_case_name(gcm_res, compiler, nx, ny, dx, EW):
    base = 'SP1_' + compiler + '_ncpl' + str(atm_ncpl)
    if EW:
        base = base + '_EW'
    if ny > 1:
...
def set_resub_and_ndays(ndays):
    """Reduce ndays and set resub if necessary for longer runs"""
    ndays_per_resub = 6
    if ndays > 7:
        resub = -(-ndays // ndays_per_resub) # ceiling division
        ndays_per_resub = -(-ndays // resub)
        resub = resub - 1
    else:
        resub = 0
        ndays_per_resub = ndays
    return resub, ndays_per_resub
...
for (nx, ny, dx, EW) in run_combos:
    ...
    if Config.build is True:
        ...
        if ny > 1:  # 3D case
            ...
            new_cam_config_opts['-cppdefs'] = "'-DSP_DIR_NS'"
    ...
    for case, ndays in ndays_to_run.iteritems():
        ...
        # update resub, ndays:
        resub, nday_sub = set_resub_and_ndays(ndays)
