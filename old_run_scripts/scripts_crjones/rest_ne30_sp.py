...
Config = runsp.Config(newcase=False, config=False, clean=True, build=True,
...
gcm_res = "ne30"
sp_base = "SP2_ECPP"
basename = sp_base + "_dCFL"
res = gcm_res  # to make sure I didn't miss anything 
# (nx, ny, dx, nlev, ncpl)
# run_combos = [(64, 1, 4, 30, 288), (64, 1, 4, 72, 288), (64, 1, 4, 30, 48), (64, 1, 4, 72, 48)]
run_combos = [(64, 1, 4, 72, 144)]
top_dir = "/ccs/home/crjones/repos/ecpp-with-cfl/"


def gen_case_name(gcm_res, compiler, nx, ny, dx, nlev, ncpl=48, EW=False, basename=basename):
...
cam_namelist_hindcast = {
    "nhtfrq": "0,-1,-24",
    "mfilt": "1,24,30",
    "fincl2": "'T','Q','U','V','PS', 'TS', 'QRL', 'QRS', 'FSNT', 'FLNT', 'FLNS', 'FSNS',"
         "'FSNTC', 'FLNTC', 'LHFLX', 'SHFLX', 'TAUX', 'TAUY', 'PRECT', 'TMQ', 'LWCF', 'SWCF', 'UBOT', 'VBOT', 'QBOT',"
         "'TBOT', 'UAP', 'VAP', 'QAP', 'QBP', 'TAP', 'TBP', 'TFIX', 'CLOUD', 'CLDLIQ', 'CLDICE', 'QEXCESS', 'PTTEND',"
         "'AEROD_v', 'EXTINCT', 'SPDT', 'SPDQ', 'SPQTFLX', 'SPQTFLXS', 'SPTLS', 'SPQTLS', 'SPQPEVP', 'SPMC', 'SPMCUP', 'SPMCDN', 'SPMCUUP',"
         "'SPMCUDN','Z3','TREFHT'",
    "fincl3": "'T','Q','Z3','OMEGA','U','V','CLOUD',"
...
for (nx, ny, dx, nlev, ncpl) in run_combos:
    ...
    # ===================================================
    BuildCase = runsp.Case(case_name=gen_case_name(gcm_res, compiler, nx, ny, dx, nlev, ncpl),
                           res=gcm_res,
                           compiler=compiler,
    ...
    if trial_run is False:
        ...
        if do_branch:
            BuildCase.xmlchanges("env_run.xml", test=trial_run,
                                 RUN_TYPE="branch",
                                 RUN_REFCASE="ne30_SP2_ECPP_intel_72L_64x1_4km_nc144",
                                 RUN_REFDATE="2000-05-10",
                                 RESUBMIT=str(resub))
        if Config.debug:
            BuildCase.xmlchanges("env_run.xml", test=trial_run,
                                 REST_OPTION="nsteps",
