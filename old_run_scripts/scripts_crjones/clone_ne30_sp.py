...
Config = runsp.Config(newcase=False, config=False, clean=False, build=False,
                      runsim=False, update_namelist=True, debug=False,
...
pre_built_combo = (64, 1, 4, 72, 48)
run_combos = [(64, 1, 4, 72, 96)]


def gen_case_name(gcm_res, compiler, nx, ny, dx, nlev, ncpl=48, EW=False):
    base = 'SP1_' + compiler
    if EW:
        base = base + '_EW'
    if ny > 1:
...
PreBuildCase = runsp.Case(case_name=gen_case_name(gcm_res, compiler, *pre_built_combo),
                          res=gcm_res,
                          compiler=compiler,
