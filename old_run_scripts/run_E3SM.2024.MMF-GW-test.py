...
src_dir = os.getenv('HOME')+'/E3SM/E3SM_SRC1' # branch => master @ 2024-02-27
...
### select case to run
# compset='F20TR-MMF1'; nlev=64; crm_nz=54; case = '.'.join(['E3SM','2023-SCIDAC',grid,compset,f'L{nlev}'])
...
# compset='F20TR-MMF1'; nlev=72; crm_nz=60;case = '.'.join(['E3SM','2024-MMF-GW-test',grid,compset,f'L{nlev}','CTL']); limit_ttend = False
compset='F20TR-MMF1'; nlev=72; crm_nz=60;case = '.'.join(['E3SM','2024-MMF-GW-test',grid,compset,f'L{nlev}','EXP']); limit_ttend = True
...
if config :
   ...
   #-------------------------------------------------------
   cpp_opt = ''
   if limit_ttend : cpp_opt += f' -DMMF_LIMIT_TTEND'
   if cpp_opt != '' :
...
if 'init_file_lnd' in locals() or 'data_file_lnd' in locals():
   ...
   run_cmd(f'./xmlchange RUN_STARTDATE=1985-01-01')
