...
submit       = True
# continue_run = True
queue = 'debug'  # regular or debug on Cori (30 min time limit for debug queue)
...
if queue=='debug' : 
   ...
   stop_opt,stop_n,resub,walltime = 'ndays',1,0,'4:00:00'
   if continue_run : resub=3
...
# case = 'E3SM_TEST-HVx2_'+cldc+'_'+res+'_'+compset+'_00'  # np4 test w/ reduced hyperviscosity
...
if submit : 
   ...
   if npg==4 : os.system("./xmlchange -file env_run.xml -id EPS_AGRID  -val 1.0e-11 ")
   #-------------------------------------------------------
   ...
   # file.write(" fincl1    = 'CRM_T','CRM_QV','CRM_W'")
   ...
   file.write(' tracer_cnst_file = \'/project/projectdirs/acme/inputdata/atm/cam/chem/trop_mozart_aero/oxid/oxid_1.9x2.5_L26_1850-2015_c20171110.nc\' \n')
   ...
   # if ne==120 and npg==2 : 
      ...
      # file.close()
      # os.system('./xmlchange -file env_run.xml      EPS_AGRID=1e-11' )
   
   nfile = 'user_nl_mpaso'
   file = open(nfile,'w') 
   file.write(" config_dt = \'00:20:00\' \n")
   file.close()

   nfile = 'user_nl_mpassi'
   file = open(nfile,'w') 
   file.write(" config_dt = 1200.0 \n")
   file.close()
   ...
   os.system('./xmlchange -file env_run.xml      OCN_NCPL='   +str(ncpl)   )
   os.system('./xmlchange -file env_run.xml      LND_NCPL='   +str(ncpl)   )
   os.system('./xmlchange -file env_run.xml      ICE_NCPL='   +str(ncpl)   )
