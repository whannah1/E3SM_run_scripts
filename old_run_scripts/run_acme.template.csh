...
set run_name       = schk_01
set job_name       = $run_name
set compset        = FC5AV1C-L
set resolution     = ne30_ne30
set machine        = titan
# setenv project       acme
setenv project       cli115

### SOURCE CODE OPTIONS
set fetch_code     = false
set acme_tag       = acme
# set tag_name       = ACME_SRC
set tag_name       = ACME-master
...
# set code_root_dir    = ~/ACME/ACME_SRC
...
### SIMULATION OPTIONS
set atm_output_freq              = -6
set records_per_atm_output_file  = 4
...
if ! ($lower_case_machine == anvil ) then #anvil has HOST blogin#... punting
...
set case_name = ACME.${compset}.${resolution}.${machine}.${run_name}    # whannah
...
if ( `lowercase $run_root_dir` == 'default' ) then
  if ( $machine == 'edison' || $machine == 'hopper' || $machine == 'cori' ) then
    ...
    set run_root_dir = /lcrc/group/acme/${USER}/ACME_simulations/${case_name}
  else
...
if ( `lowercase $short_term_archive_root_dir` == 'default' ) then
  if ( $machine == 'edison' || $machine == 'hopper' || $machine == 'cori' ) then
    ...
    set short_term_archive = /lcrc/group/acme/${USER}/archive/${case_name}
  else
...
#======================================================================================
# whannah - custom titan setup 
#======================================================================================
if (  $machine == 'titan' ) then
  set PROJWORK = /lustre/atlas1/cli115/scratch/
  # set PROJWORK = 
  
  set run_root_dir     = /lustre/atlas1/cli115/scratch/hannah6/${case_name}
  # set case_scripts_dir = $run_root_dir/case_scripts
  set case_scripts_dir = /ccs/home/hannah6/ACME/Cases/${case_name}
  # set case_build_dir   = /ccs/home/hannah6/ACME/acme_scratch/cli115/${case_name}/build
  set case_build_dir   = $run_root_dir/build
  set case_run_dir     = $run_root_dir/run

  set short_term_archive_root_dir = /lustre/atlas1/cli115/scratch/hannah6/archive/${case_name}
  set temp_case_scripts_dir = $run_root_dir/${case_name}
endif
#======================================================================================
# whannah
#======================================================================================
...
# amount of space before #PBS or #MSUB commands (which we handle via sed regular expressions). 
...
#=============================================================
# MAKE FILES AND DIRECTORIES CREATED BY THIS SCRIPT READABLE BY EVERYONE ON ACME
#=============================================================
#note that we also need to make files have acme as the group. 
#this can only be done once the run_root_dir has been created,
#so it is done later.
umask 027
...
cp env_case.xml LockedFiles/env_case.xml          # whannah
...
#./xmlchange -file env_run.xml -id PIO_NUMTASKS -val 128
...
set input_data_dir = '/lustre/atlas1/cli900/world-shared/cesm/inputdata'
...
if ( `lowercase $debug_queue` == 'true' ) then
  if ( $machine == cab || $machine == sierra ) then
    sed -i /"#MSUB \( \)*-q"/c"#MSUB -q pdebug"                     ${case_name}.run
    sed -i /"#MSUB \( \)*-l walltime"/c"#MSUB -l walltime=00:30:00" ${case_name}.run
  else if ( $machine == hopper ) then
    sed -i /"#PBS \( \)*-q"/c"#PBS  -q debug"                        ${case_name}.run
    sed -i /"#PBS \( \)*-l walltime"/c"#PBS  -l walltime=00:30:00"   ${case_name}.run
  else if ( $machine == cori || $machine == edison ) then
    sed -i /"#SBATCH \( \)*--partition"/c"#SBATCH  --partition=debug" ${case_name}.run
    sed -i /"#SBATCH \( \)*--time"/c"#SBATCH  --time=00:30:00"        ${case_name}.run
  else if ( $machine == titan || $machine == eos  ) then    
    sed -i /"#PBS \( \)*-q"/c"#PBS  -q debug"                        ${case_name}.run
    # sed -i /"#PBS \( \)*-l walltime"/c"#PBS  -l walltime=00:30:00"   ${case_name}.run
  else if ( $machine == anvil ) then 
    echo 'anvil is so small that using debug partition is silly. Exiting'
    exit 309
  else
  ...
  if ( $machine == cab || $machine == sierra ) then
    sed -i /"#MSUB\( \)*-q"/c"#MSUB  -q pbatch"                       ${case_name}.run
    sed -i /"#MSUB\( \)*-l walltime"/c"#MSUB  -l walltime=01:29:00"   ${case_name}.run
  else if ( $machine == hopper ) then
    sed -i /"#PBS \( \)*-q"/c"#PBS  -q regular"                        ${case_name}.run
    sed -i /"#PBS \( \)*-l walltime"/c"#PBS  -l walltime=02:00:00"     ${case_name}.run
  else if ( $machine == cori || $machine == edison ) then
    sed -i /"#SBATCH \( \)*--partition"/c"#SBATCH  --partition=regular" ${case_name}.run
    sed -i /"#SBATCH \( \)*--time"/c"#SBATCH  --time=02:00:00"          ${case_name}.run
### Only specially authorized people can use the special_acme qos. Don't uncomment unless you're one.
###    sed -i /"#SBATCH \( \)*--partition"/a"#SBATCH  --qos=special_acme"  ${case_name}.run
  else if ( $machine == titan || $machine == eos  ) then
    sed -i /"#PBS \( \)*-q"/c"#PBS  -q batch"                          ${case_name}.run
    sed -i /"#PBS \( \)*-l walltime"/c"#PBS  -l walltime=02:00:00"     ${case_name}.run
  else if ( $machine == anvil ) then
    sed -i /"#PBS \( \)*-l walltime"/c"#PBS  -l walltime=06:00:00"     ${case_name}.run
  else
...
if ( $machine == hopper ) then
    sed -i /"#PBS \( \)*-N"/c"#PBS  -N ${job_name}"                                ${case_name}.run
    sed -i /"#PBS \( \)*-A"/c"#PBS  -A ${project}"                                 ${case_name}.run
    ...
    sed -i /"#SBATCH \( \)*--job-name"/c"#SBATCH  --job-name=${job_name}"                 ${case_name}.run
    sed -i /"#SBATCH \( \)*--job-name"/a"#SBATCH  --account=${project}"                   ${case_name}.run
    sed -i /"#SBATCH \( \)*--output"/c"#SBATCH  --output=batch_output/"${case_name}'.o%j' ${case_name}.run
    ...
    echo 'Still need to know anvil commands...'
...
#COUPLER BUDGETS
./xmlchange -file env_run.xml -id BUDGETS     -val `uppercase $do_cpl_hist`

#COUPLER HISTORY FILES
./xmlchange -file env_run.xml -id HIST_OPTION -val `lowercase $cpl_hist_units`
./xmlchange -file env_run.xml -id HIST_N      -val $cpl_hist_num

#NUMBER OF TIMES TO RESUBMIT SIMULATION
./xmlchange -file env_run.xml -id RESUBMIT    -val $num_resubmits

#NUMBER OF DAISYCHAINED/DEPENDENT RESUBMITS
./xmlchange -file env_run.xml -id RESUBMIT_QUEUED    -val $num_depends
...
#-----------------------------------------------------------------------------------------------
  ...
  if ( ! -f $rpointer_filename ) then
    echo "run_acme: ERROR ${rpointer_filename} doesn't exist. It is needed to extract RUN_REFDATE."
