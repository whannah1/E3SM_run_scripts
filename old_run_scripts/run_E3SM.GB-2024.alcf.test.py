...
acct = 'CSC249ADSE15_CNDA'
case_dir = '/lus/gecko/projects/CSC249ADSE15_CNDA/whannah/e3sm_scratch'
src_dir  = os.getenv('HOME')+'/E3SM/E3SM_SRC0'  # branch => master + sarats/machines/frontier
...
gnz,cnz,cnx,cny,rnx,cdx,cdt,gdt_min=276,256,64,64,2,2000,1,5; nnode=12
# gnz,cnz,cnx,cny,rnx,cdx,cdt,gdt_min=148,128,64,64,2,2000,10,20; nnode=12
...
print(f'\n  case : {case}\n')
#---------------------------------------------------------------------------------------------------
hiccup_scratch = '/lus/gecko/projects/CSC249ADSE15_CNDA/whannah/hiccup_scratch'
...
if 'GPU' in arch: max_task_per_node,max_mpi_per_node,atm_nthrds  = 12,12,1
...
if newcase :
   ...
   cmd = f'{src_dir}/cime/scripts/create_newcase -case {case} --output-root {case_dir}'
   ...
   if arch=='IFXGPU' :cmd+=f' -mach sunspot -compiler oneapi-ifxgpu -pecount {atm_ntasks}x{atm_nthrds} '
   run_cmd(cmd)
   #-------------------------------------------------------
