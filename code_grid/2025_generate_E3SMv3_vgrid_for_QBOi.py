...
# parameters for doubling the resolution for QBOi vertical grid sensitivity
QBOi_double_res_altitude_min = 15e3

# ofile = 'L80_for_E3SMv3.nc'
ofile = os.getenv('HOME')+'/E3SM/vert_grid_files/Lxx_for_E3SMv3_QBOi.nc'
...
def main():
    ...
    zlev = compute_altitude_from_pressure(ilev72)
    
    # define lower limit for smoothing - do this before refinement
    smooth_limit_k = (np.abs(zlev - smooth_limit_z*1e3)).argmin()

    #---------------------------------------------------------------------------
    # refine the L72 grid by adding levels within a specified range
    zlev = refine(zlev)

    zlev = QBOi_double_res(zlev)

    # apply smoothing
    zlev = smooth(zlev,smooth_limit_k)
    #---------------------------------------------------------------------------
    # use curve fit from climatology to get pressure from height
    ilev = compute_pressure_from_altitude(zlev)
    num_ilev = len(ilev)
    num_mlev = num_ilev-1
...
#-------------------------------------------------------------------------------
def compute_altitude_from_pressure(plev):
    return np.log(plev[::-1]/1e3) * -6740.
#-------------------------------------------------------------------------------
def compute_pressure_from_altitude(zlev):
    return np.exp( -1*zlev[::-1]/(6740.) ) * 1000.
#-------------------------------------------------------------------------------
...
def refine(zlev_in):
    ...
    #---------------------------------------------------------------------------
    return compute_zlev_from_dz(dz)
#-------------------------------------------------------------------------------
def QBOi_double_res(zlev_in):
    zlev = zlev_in
    dz = compute_dz_from_zlev(zlev_in)
    #---------------------------------------------------------------------------
    for k in range(1,len(zlev_in)):
        if zlev[k]>=QBOi_double_res_altitude_min:
            k_min = k
            break
    for k in range(k_min,len(zlev_in)-1):
        kk = 2*k - k_min
        dz_tmp = dz[kk]/2.
        dz[kk] = dz_tmp
        dz.insert(kk,dz_tmp)
...
def smooth(zlev_in,smooth_limit_k):
    ...
    for k in range(0,num_mlev):
        dz_smoothed[k] = zlev_smoothed[k+1] - zlev_smoothed[k]
