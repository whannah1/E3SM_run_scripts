...
def main():
    ...
    # grid_name,refine,nsmooth = 'L72_E3SM_smooth',False, 40
    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-add+smooth',True,'add',40
    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-shift+smooth',True,'shift',40

    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-addmax+smooth',True,'addmax',40

    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-add+smooth',True,'add',0
    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-shift+smooth',True,'shift',0

    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-trim+smooth',True,'trim',40
    # grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-limit',True,'limit',0
    grid_name,refine,refine_method,nsmooth = 'L72_E3SM_refine-scale',True,'scale',0
    ...
    #---------------------------------------------------------------------------
    if refine and refine_method=='trim': nlev_orig =len(zlev)
    if refine and ( refine_method=='add' or refine_method=='trim'):
    ...
    #---------------------------------------------------------------------------
    # refinement method #1.5 - Trim off upper levels after adding levels
    #---------------------------------------------------------------------------
    if refine and refine_method=='trim':
        zlev_old = np.copy(zlev)
        zlev = np.zeros(nlev_orig)
        for k in range(nlev_orig):
            print(f'k: {k}   zlev_old[k]: {zlev_old[k]}')
            zlev[k] = zlev_old[k]

    #---------------------------------------------------------------------------
    # refinement method #2 - shift levels
    #---------------------------------------------------------------------------
    if refine and refine_method=='shift':
        ...
        ### iterate multiple times over algorithm o ruse while loop?
        for n in range(20):
        
            ### loop over refinement region to reduce spacing proportional to difference relative to 
            delta_dz_total = 0
            for k in range(refine_limit_k1,refine_limit_k2): 
                # only refine if spacing is sufficiently large
                if dz[k] > min_dz:
                    delta_dz = ( dz[k] - min_dz ) * 0.2
                    dz[k] -= delta_dz
                    delta_dz_total += delta_dz
            
            # if no change then we're done
            # if delta_dz_total==0: break

            ### above main region add spacing as constant to preserve model top
            num_augment = len(zlev) - 1 - refine_limit_k2
            delta_dz_augment = delta_dz_total/num_augment
            for k in range(refine_limit_k2,len(zlev)-1): 
                dz[k] += delta_dz_augment

            ## reconstruct the levels
            zlev = np.zeros(len(dz)+1)
            for k in range(1,len(dz)+1): zlev[k] = zlev[k-1]+dz[k-1]

        # print(f'max(zlev_old): {np.max(zlev_old)}')
        # print(f'max(zlev_new): {np.max(zlev)}')
        print_levs(zlev, zlev_old, dz, dz_old, refine_limit_k1, refine_limit_k2)

    # exit()

    #---------------------------------------------------------------------------
    # refinement method #3 - limit rate of stretching
    #---------------------------------------------------------------------------
    if refine and refine_method=='limit':
        ds72_alt = xr.open_dataset(f'{home}/E3SM/vert_grid_files/L72_E3SM_smooth.nsmooth_40.upper_lev_only.nc')

        am72_alt, bm72_alt = ds72_alt.hyam.values, ds72_alt.hybm.values
        ai72_alt, bi72_alt = ds72_alt.hyai.values, ds72_alt.hybi.values
        mlev72_alt = compute_pressure_from_hybrid_coef(am72_alt,bm72_alt)
        ilev72_alt = compute_pressure_from_hybrid_coef(ai72_alt,bi72_alt)
        zlev_alt = np.log(ilev72_alt[::-1]/1e3) * -6740.

        # find max stretching rate in reference grid
        dzdz_limit = 0
        for k in range(1,72-1):
            dzdz = ( zlev_alt[k+1] - zlev_alt[k] ) - ( zlev_alt[k] - zlev_alt[k-1] )
            if dzdz>dzdz_limit: dzdz_limit = dzdz

        # apply limiting of dzdz in current grid
        for k in range(1,72-1):
            dzdz = ( zlev[k+1] - zlev[k] ) - ( zlev[k] - zlev[k-1] )
            if dzdz>dzdz_limit:
                dz_correction = dzdz - dzdz_limit
                zlev[k+1:] = zlev[k+1:] - dz_correction

        for k in range(len(zlev)-1): dz[k] = zlev[k+1]-zlev[k]
        
    #---------------------------------------------------------------------------
    # refinement method #4 - scale grid to limit dz and maintain structure
    #---------------------------------------------------------------------------
    if refine and refine_method=='scale':
        ds72_alt = xr.open_dataset(f'{home}/E3SM/vert_grid_files/L72_E3SM_smooth.nsmooth_40.upper_lev_only.nc')
        ...
        dz_alt = []
        for k in range(len(zlev)-1): dz_alt.append(zlev_alt[k+1]-zlev_alt[k])
        dz_alt = np.array(dz_alt)

        # define limits for calculating mean dz
        scale_limit1_z_km = 20
        # scale_limit2_z_km = 30
        scale_limit_k1 = (np.abs(zlev - scale_limit1_z_km*1e3)).argmin()
        # scale_limit_k2 = (np.abs(zlev - scale_limit2_z_km*1e3)).argmin()

        # print(scale_limit_k1); exit()
        scale_limit_k1 = 50


        def print_stat(x):
            print(f'  {np.min(x):8.2f}  {np.mean(x):8.2f}  {np.max(x):8.2f}')

        # # this block verifies how to construct zlev from dz
        # print_stat(zlev)
        # for k in range(1,len(zlev)): 
        #     zlev[k] = zlev[k-1] + dz[k-1]
        # print_stat(zlev)
        # exit()

        print_stat(dz_alt)
        # print_stat(dz_alt[scale_limit_k1:scale_limit_k2])
        print_stat(dz_alt[scale_limit_k1:])

        print()
        print_stat(dz)
        # print_stat(dz[scale_limit_k1:scale_limit_k2])
        print_stat(dz[scale_limit_k1:])

        # ratio = np.mean(dz_alt[scale_limit_k1:scale_limit_k2]) / np.mean(dz[scale_limit_k1:scale_limit_k2])
        ratio = 0.89
        ...
        dz[scale_limit_k1:] = dz[scale_limit_k1:] * ratio

        # zlev[scale_limit_k1:] = zlev[scale_limit_k1:] * 0.95
        # for k in range(len(zlev)-1): dz[k] = zlev[k+1]-zlev[k]
    ...
    #---------------------------------------------------------------------------
    # refinement method #5 - reset everything above a certain height to the minimum spacing
    #---------------------------------------------------------------------------
    if refine and refine_method=='addmax':
        # for k in range(len(zlev)-1): print(f'k: {k:3d}   {zlev[k+1]:6.2f}  {dz[k]:6.2f}'); print()
        ...
        ### breakup top layer into N layers of min_dz
        num_new_layers = int(np.floor(dz[-1]/min_dz))
        dz[-1] = min_dz
        for k in range(refine_limit_k2+1,refine_limit_k2+num_new_layers): 
            dz = np.insert(dz,k,min_dz)

        if len(dz)>128: dz = dz[0:128]
    ...
    # ofile = os.getenv('HOME')+f'/E3SM/vert_grid_files/{grid_name}.nsmooth_{nsmooth}.plim_{limit_p}.nc'
    # ofile = os.getenv('HOME')+f'/E3SM/vert_grid_files/{grid_name}.nsmooth_{nsmooth}.zlim_{int(limit_z_km)}km.nc'
    if nsmooth==0:
        ofile = os.getenv('HOME')+f'/E3SM/vert_grid_files/{grid_name}.nc'
    else:
        ofile = os.getenv('HOME')+f'/E3SM/vert_grid_files/{grid_name}.nsmooth_{nsmooth}.upper_lev_only.nc'
